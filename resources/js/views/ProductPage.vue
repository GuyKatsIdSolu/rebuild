  ...
  <div class="" v-if="img">
    ...
    <div class="product-page shop">
      <div class="categories-filter">
        ...
        <a v-for="(categoryId,index) in categories" :key="categoryId" href="" @click.prevent="$router.push('/product-'+productsBycategory[categoryId][Object.keys(productsBycategory[categoryId])[0]].id)">
      ...
      <div class="container main-page row">
        <transition-group v-if="!$root.isMobile" name="fade" tag="div" class="thumbnails">
          <a v-for="(n, i) in product.details.dynamic_previews_number" @click.prevent="curMainPrev='/storage/creator_images/'+img.id+'/previews/'+product.product_code+'/1000_'+(i+1)+color+'.jpg'" :key="i">
            <v-lazy-image :class="curMainPrev=='/storage/creator_images/'+img.id+'/previews/'+product.product_code+'/1000_'+(i+1)+color+'.jpg'? 'active' : ''" :src="'/storage/creator_images/'+img.id+'/previews/'+product.product_code+'/500_'+(i+1)+color+'.jpg'" :src-placeholder="'/storage/creator_images/'+img.id+'/previews/'+product.product_code+'/80_'+(i+1)+color+'.jpg'" />
          </a>
          <a v-for="(n, i) in (4-product.details.dynamic_previews_number)" @click.prevent="curMainPrev='/storage/gallery_static_images/'+product.product_code+'_'+(i+1)+'.jpg'" :key="i+product.details.dynamic_previews_number">
            <v-lazy-image :class="curMainPrev=='/storage/gallery_static_images/'+product.product_code+'_'+(i+1)+'.jpg'? 'active' : ''" :src="'/storage/gallery_static_images/'+product.product_code+'_'+(i+1)+'.jpg'" src-placeholder="/storage/images/placeholder-white.png" />
          </a>
        </transition-group>
        <div class="big-img" v-if="!$root.isMobile">
          <enlargeable-image trigger="click" animation_duration="100" class="enlarge-my" :src="$root.currentUrl + '/storage/creator_images/'+img.id+'/previews/'+product.product_code+'/80_1.jpg'" :src_large="$root.currentUrl + '/storage/creator_images/'+img.id+'/previews/'+product.product_code+'/1000_1.jpg'">
          ...
          :media="$root.currentUrl + '/storage/creator_images/'+img.id+'/previews/'+product.product_code+'/1000_1.jpg'">
      ...
      <div class="details" v-if="!$root.isMobile">
        ...
        <div class="delivery">
          <img src="/storage/images/car-ic.png" alt="">
            ...
            <div>
              <a href="" @click.prevent="product=upsaleProduct;refreshImgsListForSlider();scrollToTop()" >
                <div class="gallery-item-frame">
                  <v-lazy-image class="prev-gallery-img" :src="'/storage/creator_images/'+img.id+'/previews/'+upsaleProduct.product_code+'/500_1.jpg'" :src-placeholder="'/storage/creator_images/'+img.id+'/previews/'+upsaleProduct.product_code+'/80_1.jpg'"/>
                </div>
          ...
          <slide v-for="(product,imgIdName,index) in imgsListForSlider" :key="index" class="gallery-item ">
            <div>
              <a href="" @click.prevent="$router.push('/product-' + product.id);scrollToTop()" >
                <div class="gallery-item-frame">
                  <v-lazy-image class="prev-gallery-img" :src="'/storage/creator_images/' + imgIdName.split('-')[0] + '/previews/'+product.details.product_code+'/500_1.jpg'" :src-placeholder="'/storage/creator_images/' + imgIdName.split('-')[0] + '/previews/'+product.details.product_code+'/80_1.jpg'" />
                </div>
                <div class="gallery-item-desc">
      ...
      <hooper style="height:200zpx" :wheelControl="false" :centerMode="true" :itemsToShow="5.5" :infiniteScroll="true" >
        <slide  v-for="(image,index) in img.creator.images" :key="index" class="item" style="  ">
          <a href="" @click.prevent="$router.push('/catalog-'+image.id)">
            <div class="frame">
              <v-lazy-image :src="'/storage/creator_images/'+image.id+'/500.jpg'" :src-placeholder="'/storage/creator_images/'+image.id+'/80.jpg'" />
            </div>
          </a>
        </slide>
        <hooper-navigation slot="hooper-addons" ></hooper-navigation>
...
export default {
  ...
  watch:{
    ...
    product: function(newVal, oldVal) {
      ...
      this.quantity=1;
      this.curMainPrev='/storage/creator_images/'+this.product.image_id+'/previews/'+this.product.product_code+'/1000_1.jpg';
      this.selectedProperties={};
  ...
  methods:{
    ...
    chooseColor(optionName){
      ...
      this.curMainPrev='/storage/creator_images/'+this.product.image_id+'/previews/'+this.product.product_code+'/1000_1'+optionName+'.jpg';
    ...
    addToCart(goCheckoutPage=false) {
      ...
      axios.post('/api/add-item-to-cart', {
        ...
        'previewUrl': this.$root.currentUrl + '/storage/creator_images/'+this.img.id+'/previews/'+this.product.product_code+'/1000_1.jpg',
    ...
    getProductPreviews() {
      var that = this;
      axios.get('/api/image-by-product-id/' + this.$route.params.productId)
      .then(response => {
        ...
        that.img.products.forEach((product, i) => {
          if(that.$route.params.productId==product.id){
            ...
            that.curMainPrev = '/storage/creator_images/'+that.product.image_id+'/previews/'+that.product.product_code+'/1000_1.jpg';
          }
