<template>
  <div>
    <div class="gallery-box" v-if="creator">
      ...
      <div v-if="status=='approved' || forceSearch" style="    margin-bottom: 15px;display: inline-block;">
        ...
        <masonry :cols="{default: 4, 1000: 3, 700: 2, 400: 2}" :gutter="{default: '20px', 400: '40px'}">
          ...
          <div :style="status=='approved' || status=='not approved' ? '' : 'padding: 0 0 30px 0;'" :class="'img-box _1Nk0C '+isNotApproved(img)" v-for="(img, $index) in imgs" :key="$index" v-if="$index < limit">
            <div :style="status=='approved' || status=='not approved' ? '' : 'height: calc(100% - 30px)'" class="hover" @click="$root.isMobile? '' :goToShop(img.id)">
              ...
              :title="img.name + ' (ID '+img.id+')'+' '+img.description+ ' ' +getPinterestContent(img)"
              :media="getSrc(img)"
              >
      ...
      <div class="center" v-if="pagination">
        <div class="relative-pagination">
          <pagination :data="imgs" @pagination-change-page="getResults">
            <span class="prev" slot="prev-nav">Previous</span>
            <span class="next" slot="next-nav">Next</span>
          </pagination>
        </div>
      </div>
      <div class="see-all-box" v-if="(!pagination) && (imgs) && (imgs.length > limit-1)">
...
export default {
  ...
  methods: {
      ...
      setTimeout(function(){
        if (that.status=='approved'){
          if (!that.$root.clicked){
            // window.location.href = window.location.origin.replace('https://my.', 'https://shop.') + 'gallery/' + store_id;
    ...
    deleteImg(index, imgId) {
      ...
      this.$root.clicked = true;
      var that = this;
      axios.get('/api/delete-img/'+imgId)
      .then(response => {
    ...
    getResults(page = 1, search = '') {
      ...
      axios.get('/api/get-user-images', {
        ...
        if(this.page==1 && !this.curData){
          this.curData=response.data.imgs;
        }
      });
    },
    checkNewResults() {
      axios.get('/api/get-user-images', {
        ...
        this.newData = response.data.imgs;
        if(this.search==''){
          this.tags = response.data.tags;
          this.total = response.data.imgs.total;
        }
